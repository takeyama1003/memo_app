<?php
echo '<table>';
echo '<tr><th>ID</th><th>タイトル</th><th>本文</th><th>カテゴリー</th><th></th></tr>';
...
//カテゴリー検索
	...
	// $sql=$pdo->prepare('select * from memo_category2 where category_name like ?');
	// $sql->execute(['%'.$_REQUEST['category_name'].'%']);
}
else {
	//全表示
	// $sql=$pdo->query('select * from memo_category2');
